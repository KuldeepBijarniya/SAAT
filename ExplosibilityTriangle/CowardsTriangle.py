...
class ExplosibilityTriangle:
    def plot_cowards_triangle(sealed_volume, leakage_coefficient, time_step, total_time, methane_inflow_rate, inert_gas_inflow_rate, inert_gas_purity, air_purity, average_pressure, max_pressure_change, pressure_period, pressure_time_offset, oxygen_depletion_rate, carbon_dioxide_inflow_rate, carbon_dioxide_production_rate, initial_methane, initial_nitrogen, initial_oxygen, initial_CO2):
        ...
        initial_methane_percentage = initial_methane / (initial_methane + initial_oxygen + initial_nitrogen + initial_CO2) * 100
        initial_oxygen_percentage = initial_oxygen / (initial_methane + initial_oxygen + initial_nitrogen + initial_CO2) * 100

        # Add initial concentrations to the trajectory
        x_triangle = [initial_methane_percentage] + list(methane_percentage)
        y_triangle = [initial_oxygen_percentage] + list(oxygen_percentage)

        # Plot trajectory in the Cowards Triangle with a thin line
        plt.plot(x_triangle, y_triangle, color='black', linewidth=2)

        # Annotate the final point with a smaller arrow
        plt.annotate('', xy=(x_triangle[-1], y_triangle[-1]), xytext=(x_triangle[-2], y_triangle[-2]), arrowprops=dict(arrowstyle='->', shrinkA=5, shrinkB=5, color='black', linewidth=3))
